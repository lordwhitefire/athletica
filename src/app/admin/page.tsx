import { getDashboardCounts } from "@/lib/getDashboardCounts";
...
export default async function AdminDashboard() {
    ...
    if (result.error) {
        return (
            <div>
                ...
                <p className="text-red-500 text-sm mb-6">{result.error.message}</p>
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-10">
                    {statCardsWithValues(0, 0, 0, 0)}
                </div>
    ...
    const { productCount, brandCount, navCount, linkCount } = result.data;
    ...
    return (
        <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-10">
                {statCardsWithValues(productCount, brandCount, navCount, linkCount)}
            </div>
...
function statCardsWithValues(productCount: number, brandCount: number, navCount: number, linkCount: number) {
    const cards = [
