...
function L3Item({ l3Item, onClose }: { l3Item: NavItem; onClose: () => void }) {
    ...
    return (
        <div className="relative">
            <button
                className={`w-full flex items-center justify-between py-1 font-bold text-xs uppercase transition-colors ${isOpen ? "text-primary" : "text-on-surface"
                    }`}
...
function L2Item({ l2Item, onClose }: { l2Item: NavItem; onClose: () => void }) {
    ...
    return (
        <div className="mt-2">
            <button
                className={`w-full flex items-center justify-between pl-12 pr-4 py-3 font-semibold text-xs tracking-wider uppercase transition-colors ${isOpen ? "text-on-surface" : "text-neutral-500 opacity-80"
                    }`}
                onClick={() => setIsOpen(!isOpen)}
            >
    ...
    return (
        <div className="absolute inset-0 bg-white flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto no-scrollbar pb-32 px-4 space-y-1">
                ...
                <div className="flex items-center gap-3 p-4 text-neutral-700 font-headline font-medium text-sm tracking-tight uppercase hover:pl-6 transition-all duration-300 cursor-pointer">
                ...
                <div className="bg-surface-container-low rounded-lg">
                    {/* L1 header */}
                    <div className="w-full flex items-center justify-between p-4 text-primary font-headline font-bold text-sm tracking-tight uppercase">
                        <div className="flex items-center gap-3">
                            <span className="material-symbols-outlined text-lg">sports_soccer</span>
                            <span>{item.label}</span>
                        </div>
                        <span className="material-symbols-outlined rotate-180 transition-transform">
                            expand_more
                        </span>
                    </div>
...
export default function MobileNav({ navigation, isOpen, onClose }: MobileNavProps) {
    ...
    return (
        <>
            ...
            {/* Drawer shell */}
            <aside
                className={`fixed top-0 left-0 h-full w-[400px] z-50 overflow-hidden shadow-2xl shadow-black/50 transition-transform duration-300 ease-in-out ${isOpen ? "translate-x-0" : "-translate-x-full"
                    }`}
            >
