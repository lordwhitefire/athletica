...
export default function Asymmetric2Split({ items }: Props) {
    ...
    return (
        ...
        >
            <Link href={items[0].link || "/"} className={`relative overflow-hidden group h-48 md:h-full ${items[0].accent || "border-r-[24px] border-primary-container"}`}>
                {items[0].image ? (
                    <Image 
                        src={items[0].image} 
                        alt={items[0].label} 
            ...
            <Link href={items[1].link || "/"} className={`relative overflow-hidden group h-48 md:h-full ${items[1].accent || "border-l-[24px] border-black"}`}>
                {items[1].image ? (
                    <Image 
                        src={items[1].image} 
                        alt={items[1].label} 
